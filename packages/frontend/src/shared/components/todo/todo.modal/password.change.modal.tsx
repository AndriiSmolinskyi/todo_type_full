...
const PasswordChangeModal: React.FC<PasswordChangeModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (values: { newPassword: string }, { resetForm }: { resetForm: () => void }) => {
    await updatePassword(values.newPassword);
    resetForm();
    onClose();
  };

  const INITIAL_VALUES = { newPassword: '' };
  ...
  return (
    <div className={styles.modalOverlayStyle}>
      <div className={styles.modalContentStyle}>
        ...
        <Formik initialValues={INITIAL_VALUES} validationSchema={PasswordChangeSchema} onSubmit={handleSubmit}>
          {({ errors, touched }) => (
            <Form className={styles.formBlock}>
              ...
              />
              <button type="submit" className={styles.modalFormSubmit}>Change Password</button>
            </Form>
          )}
        </Formik>
      </div>
