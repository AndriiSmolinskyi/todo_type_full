...
import { AuthLoginData, AuthRegisterData, AuthVerifyData, AuthResetPasswordData } from '~shared/interface/auth.interface';
...
class AuthService extends HttpService {
  ...
  public async login(data: AuthLoginData) {
    const response = await this.post<LoginResponse, AuthLoginData>('users/login', data);
  ...
  public async initiateResetPassword(email: string) {
    return this.post<null, { email: string }>('users/initiate-reset-password', { email });
  }
